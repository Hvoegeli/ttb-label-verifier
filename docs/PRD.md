...
- [MVP2] **Field extraction.** One Claude vision call returns the regulated fields as structured JSON: the detected beverage type, brand name, class/type designation, alcohol content, net contents, name and address, country of origin (when stated) with an import indicator, and the government warning text. The read is universal (it reads every beverage's fields), so the type does not need to be known in advance. Each field carries an extraction confidence signal.
...
- [MVP15] **Beverage type auto-detection and routing.** The vision read classifies the label as distilled spirits, wine, or a malt beverage, and the pipeline routes it to the matching rule set automatically, so a mixed pile of labels (the real importer case) is each judged by the right rules. The agent may force a specific type to override the detection; the result shows what type the label was judged as. This removes the friction of a wine or beer label being checked against the wrong rule set when one type is selected for a whole batch.
- [MVP14] **Country of origin check (conditional, shared across beverages).** When the product appears to be an import, confirm a country of origin is stated per 27 CFR 5.69 (spirits) and 7.69 (malt beverages), which defer to U.S. Customs marking rules at 19 CFR part 134; wine relies on the CBP rule directly. Because import status is hard to prove from one photo, this check never auto FAILs on that inference: it returns NEEDS REVIEW when the label looks imported but states no origin, and PASS for a clearly domestic label or when an origin is stated. The trade off (an unrecognized import can pass) is documented; a human spot checks cleared labels.
