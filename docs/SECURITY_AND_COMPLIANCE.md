...
8. **Human in the loop (essential for photo input).** The tool assists; it never makes the final compliance decision. Any verdict, including a FAIL, can be overridden by the agent. This matters most for photos of real bottles: testing against real labels showed that reading the verbatim Government Warning off a curved or small-print back label is the weak point, and the model can mis-transcribe it while reporting that it read it cleanly. So a photo submitted from a real bottle should always be confirmed by a person. The result page states this and, when the warning does not match, shows what the statute expects next to what was read so a reviewer can tell a real defect from a mis-read.
