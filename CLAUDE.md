...
- Beverage type is auto-detected: the vision read is universal (one schema reads every beverage's fields plus a detected `beverage_type`), and `_classify` decides the rule set AND records how (`forced` = user picked, `detected` = auto + valid read, `undetermined` = auto + null/out-of-enum). An explicit selection overrides; "auto" trusts a valid detection; an undetermined type is checked under spirits as a default book but the verdict is forced to NEEDS REVIEW, never trusted. The UI must report the source honestly and never claim a classification on a failed read. Keep extraction type-agnostic; do not reintroduce a per-beverage schema.
